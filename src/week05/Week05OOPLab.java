...
public class Week05OOPLab {
	...
	public static void main(String[] args) {
		...
		int numPlayers = 2;
		gameBoard = dealToPlayers(numPlayers);
		
		for (int i = 1; i <= numPlayers; i++) {
			String playerName = "Player " + i;
			System.out.println(playerName + "\n--------");
			List<Card> playerList = gameBoard.get(playerName);
			for (Card playerCard : playerList) {
				playerCard.describe();
			}
		}
	...
	public static Map<String, List<Card>> dealToPlayers(int numPlayers){
		...
		for (int i = 1; i <= numPlayers; i++) {
			...
			String playerName = "Player " + i;
			playerHands.put(playerName, playerList);	
		}
		for (int i = 0; i < 52/numPlayers; i++) {
			for (int j = 1; j <= numPlayers; j++) {
				String playerName = "Player " + j;
				List<Card> playerList = playerHands.get(playerName);
				playerList.add(deck.draw());
				playerHands.replace(playerName, playerList);
			}
		}
		return playerHands;
	}
